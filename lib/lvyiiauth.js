...
function createUserAuthFunctionRouter(options) {
  ...
  authFunctions.use('/me', function (req, res) {
  ...
  authFunctions.use('/loginWithMobilePhone', function (req, res) {
    if (LYAUTH._callback.loginWithMobilePhone) {
      var mobilephone = req.body.mobilephone
      var password = req.body.password
      promiseTry(() => {
        return LYAUTH._callback.loginWithMobilePhone(mobilephone, password)
      }).then((response) => {
        responseJson(res, response)
      }).catch(err => {
        responseException(res, req.url, err)
  ...
  authFunctions.use('/loginWithUsername', function (req, res) {
    if (LYAUTH._callback.loginWithUsername) {
      var username = req.body.username
      var password = req.body.password
      promiseTry(() => {
        return LYAUTH._callback.loginWithUsername(username, password)
      }).then((response) => {
        responseJson(res, response)
      }).catch(err => {
        responseException(res, req.url, err)
