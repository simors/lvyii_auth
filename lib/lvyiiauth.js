...
function createUserAuthFunctionRouter(options) {
  ...
  cloudFunctions.use(require('../middleware/lvyii-headers')(LY)());
...
const userRequestResetPasswordBySmsCode = function (req, res) {
  if (LYAUTH._callback.requestResetPasswordBySmsCode) {
    var mobilephone = req.body.mobilephone
    promiseTry(() => {
      if (req.LY.sessionToken && req.LY.sessionToken !== '') {
        return req.LY.sessionToken;
      ...
      var userId = payload.id
      return LYAUTH._callback.requestResetPasswordBySmsCode(userId, mobilephone)
    }).then((response) => {
      responseJson(res, response)
    }).catch(err => {
      responseException(res, req.url, err)
...
const userResetPasswordBySmsCode = function (req, res) {
  if (LYAUTH._callback.resetPasswordBySmsCode) {
    var assword = req.body.password
    var code = req.body.code
    promiseTry(() => {
      if (req.LY.sessionToken && req.LY.sessionToken !== '') {
        return req.LY.sessionToken;
      ...
      var userId = payload.id
      return LYAUTH._callback.resetPasswordBySmsCode(userId, password, code)
    }).then((response) => {
      responseJson(res, response)
    }).catch(err => {
      responseException(res, req.url, err)
